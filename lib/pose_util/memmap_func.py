...
def dump(obj, file_path, use_cpickle=False):
  ...
  if use_cpickle:
    ...
    joblib.dump(obj, file_path) # Automatically uses pickle.HIGHEST_PROTOCOL

def load(file_path, use_cpickle=False, mmap_mode=None):
  if does_numpy_path_have_memmap_version(file_path):
    file_path = get_memmap_path_from_numpy_path(file_path)
  ...
  if is_numpy_memmap_file_path(file_path):
    ...
    with open(file_path, "rb") as f:
      return cPickle.load(f) # Will automatically know the protocol from the file itself
  else:
