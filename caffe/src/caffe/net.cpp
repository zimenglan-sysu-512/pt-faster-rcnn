...
void Net<Dtype>::CopyTrainedLayersFrom(const NetParameter& param) {
  ...
  for (int i = 0; i < num_source_layers; ++i) {
    ...
    // 
    int t_layer_id = 0;
    std::vector<int> t_layer_ids;

    // Start -- match by the layer name
    int target_layer_id = 0;
    while (target_layer_id != layer_names_.size() &&
        layer_names_[target_layer_id] != source_layer_name) {
      ++target_layer_id;
    ...
    // If not found -- Origin
    bool no_match = target_layer_id >= layer_names_.size();
    // Check no need copy
    if(!no_match) {
      bool h_no_need_copy = 
          layers_[target_layer_id]->layer_param().has_no_need_copy();
      if(h_no_need_copy && 
          layers_[target_layer_id]->layer_param().no_need_copy()) {
        LOG(INFO) 
            << "Does not need copying source layer (" << source_layer_name 
            << ") from pretrained layer to the target layer (" 
            << layer_names_[target_layer_id] << ")";
        continue;
      }
    }
    LOG(INFO) << "Stage 001";
    // Check the endwith string
    if(no_match) {
      while(t_layer_id != layer_names_.size()) {
        bool h_c_postfix_name = 
            layers_[t_layer_id]->layer_param().has_copy_postfix_name();
        if(h_c_postfix_name) {
          string t_layer_name = layer_names_[t_layer_id];
          string c_postfix_name = 
              layers_[t_layer_id]->layer_param().copy_postfix_name();
          // Check whether source layer name endwiths copy postfix name
          int tl = t_layer_name.length();
          int cl = c_postfix_name.length();
          if(tl <= cl) {
            t_layer_id++;
            continue;
          }
          string s_c_postfix_name = t_layer_name.substr(tl - cl, cl);
          if(source_layer_name == c_postfix_name && 
              c_postfix_name == s_c_postfix_name) {
            // Check no need copy
            bool h_no_need_copy = 
                layers_[t_layer_id]->layer_param().has_no_need_copy();
            if(h_no_need_copy && 
                layers_[t_layer_id]->layer_param().no_need_copy()) {
              LOG(INFO) 
                  << "***Does not need copying source layer (" 
                  << source_layer_name 
                  << ") from pretrained layer to the target layer (" 
                  << layer_names_[target_layer_id] << ")";
              t_layer_id++;
            ...
            // Need copy
            t_layer_ids.push_back(t_layer_id);
          }
        }
        t_layer_id++;
      }
    ...
    LOG(INFO) << "Stage 002";
    // If not found -- user Define
    bool no_match2 = t_layer_ids.size() <= 0;
    
    // Copy
    if(!no_match) {
      LOG(INFO) << "Stage 3 -- Origin";
      // Check blob param
      DLOG(INFO) << "Copying source layer " << source_layer_name;
      vector<shared_ptr<Blob<Dtype> > >& target_blobs =
          layers_[target_layer_id]->blobs();
      ...
      for (int j = 0; j < target_blobs.size(); ++j) {
        if (!target_blobs[j]->ShapeEquals(source_layer.blobs(j))) {
          ...
          LOG(FATAL) << "Cannot copy param " << j 
              << " weights from layer '"
              << source_layer_name 
              << "'; shape mismatch.  Source param shape is "
              << source_blob.shape_string() 
      ...
      LOG(INFO) << "Stage 3 -- User Define";
      for(int tn = 0; tn < t_layer_ids.size(); tn++) {
        // Get layer id
        int t_layer_id2 = t_layer_ids[tn];
        // Check blob param
        DLOG(INFO) << "Copying source layer " << source_layer_name;
        vector<shared_ptr<Blob<Dtype> > >& target_blobs =
            layers_[t_layer_id2]->blobs();
        CHECK_EQ(target_blobs.size(), source_layer.blobs_size())
            << "Incompatible number of blobs for layer " 
            << source_layer_name;
        // Copy
        for (int j = 0; j < target_blobs.size(); ++j) {
          if (!target_blobs[j]->ShapeEquals(source_layer.blobs(j))) {
      ...
      LOG(INFO) << "Unknown Copy Strategy...";
    }
  }
  LOG(INFO) << "End Finishing Copying...";
}
